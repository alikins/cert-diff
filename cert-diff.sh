...
FILE2_TXT="$FILE2.txt"
openssl x509 -text -in $FILE2 > $FILE2.txt
...
/usr/lib64/nss/unsupported-tools/pp -t certificate -i $FILE1_DER > $FILE1_PP


FILE2_PP="$FILE2.pp"
FILE2_DER="$FILE2.der"
$DUMPDER $FILE2 > $FILE2_DER
/usr/lib64/nss/unsupported-tools/pp -t certificate -i $FILE2_DER > $FILE2_PP
